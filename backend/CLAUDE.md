...
**Not yet wired into the SvelteKit frontend** — `../src/lib/events`/`../src/routes/(app)/events` still call Prisma directly. Also not ported this pass (see DESIGN.md): push notifications on going/interested, the calendar range endpoint, the ICS subscribe feed, the typeahead search endpoint, the admin all-events bypass listing, nollning event visibility.
...
This section describes the article integration specifically — it's the only feature actually wired into the frontend so far (see "Events routes" above for why events isn't yet, even though its Go API exists).
