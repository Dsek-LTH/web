...
const getImageUrls = (shortName: string) => ({
  lightImageUrl:
    `https://raw.githubusercontent.com/Dsek-LTH/grafik/main/committee_logos/${shortName}/SVG/full/light.svg`,
  darkImageUrl:
    `https://raw.githubusercontent.com/Dsek-LTH/grafik/main/committee_logos/${shortName}/SVG/full/dark.svg`,
  monoImageUrl:
    `https://raw.githubusercontent.com/Dsek-LTH/grafik/main/committee_logos/${shortName}/SVG/full/bw.svg`,
  symbolUrl:
    `https://raw.githubusercontent.com/Dsek-LTH/grafik/main/committee_logos/${shortName}/SVG/symbol/dark.svg`,
});

export const DOORS = [
  { nameSv: "border", verboseName: "Border" },
  { nameSv: "ful", verboseName: "Fulförrådet" },
  { nameSv: "buren", verboseName: "Buren" },
  { nameSv: "idet", verboseName: "iDét" },
  { nameSv: "koket", verboseName: "Köket" },
  { nameSv: "komitea", verboseName: "Komitea" },
  { nameSv: "mauer", verboseName: "Mauer (caféförrådet)" },
  { nameSv: "sex", verboseName: "Sexförrådet" },
  { nameSv: "stad", verboseName: "Städförrådet" },
  { nameSv: "styrelserummet", verboseName: "Råsenbad (styrelserummet)" },
  { nameSv: "utskott", verboseName: "Utskott" },
];

export const BOOKABLES = [
  { nameSv: "Uppehållsdelen av iDét" },
  { nameSv: "Köket" },
  { nameSv: "Filmkamera" },
  { nameSv: "Lysrör" },
  { nameSv: "Sektionsbilen" },
  { nameSv: "Lila Soundboks" },
  { nameSv: "Råsa Soundboks" },
  { nameSv: "Styrelserummet", id: "99854837-fdb9-4dba-85fc-86a5c514253c" },
];

export const COMMITTEES = [
  { nameSv: "Skattmästeriet", shortName: "skattm" },
  { nameSv: "Informationsutskottet", shortName: "infu" },
  { nameSv: "Studierådet", shortName: "srd" },
  { nameSv: "Cafémästeriet", shortName: "cafe" },
  { nameSv: "Näringslivsutskottet", shortName: "naru" },
  { nameSv: "Källarmästeriet", shortName: "km" },
  { nameSv: "Aktivitetsutskottet", shortName: "aktu" },
  { nameSv: "Sexmästeriet", shortName: "sexm" },
  { nameSv: "Nollningsutskottet", shortName: "nollu" },
  { nameSv: "Framtidsutskottet", shortName: "fram" },
  { nameSv: "D-chip", shortName: "dchip" },
  { nameSv: "Medaljelelekommittén", shortName: "medalj" },
  { nameSv: "Trivselrådet", shortName: "trivsel" },
  { nameSv: "Valberedningen", shortName: "valb" },
  { nameSv: "Tackmästeriet", shortName: "tackm" },
  { nameSv: "Centralprocessutskottet", shortName: "cpu" },
...
export const MARKDOWNS = [
  ...COMMITTEES.map((committee) => ({ name: committee.shortName })),
  {
    name: "for-foretag",
    markdownSv: `[![](${
      getFileUrl("minio/news/public/b76f498a-ae6f-4af6-8273-f975bceceb05.png")
...
export const ACCESS_POLICIES = [
  ...
  ...getObjectValues(
    Array.from(
      { length: 5 },
      (_, i) => apiNames.MARKDOWNS.PAGE(`cafe:open:${i}`),
    ),
  ),
].map((apiName: string) => ({
  apiName,
...
export const models: SeedClientOptions["models"] = {
  member: {
    data: {
      ...
      studentId: (ctx) =>
        `${ctx.data.firstName?.substring(0, 2).toLowerCase()}${
          faker.number.int({ min: 1000, max: 9999 })
        }${ctx.data.lastName?.substring(0, 2).toLowerCase()}-s`,
      picturePath: () => faker.image.avatar(),
      email: (ctx) =>
        `${ctx.data.firstName?.toLowerCase()}.${ctx.data.lastName?.toLowerCase()}.${
          ctx.data.studentId?.substring(2, 6)
        }@user.dsek.se`,
      nickname: () =>
  ...
  article: {
    data: {
      bodySv: () => faker.lorem.paragraphs({ min: 3, max: 10 }),
      headerSv: () => faker.lorem.sentence(),
      createdAt: () => faker.date.past(),
      publishedAt: (ctx) => ctx.data.createdAt ?? faker.date.past(),
      slug: (ctx) => faker.helpers.slugify(ctx.data.headerSv),
      removedAt: null,
  ...
  event: {
    data: {
      titleSv: () => faker.lorem.sentence(),
      descriptionSv: () => faker.lorem.paragraphs({ min: 3, max: 6 }),
      startDatetime: () =>
      ...
      endDatetime: (ctx) =>
        dayjs(ctx.data.startDatetime)
          ...
          .toDate(),
      slug: (ctx) => faker.helpers.slugify(ctx.data.titleSv),
      shortDescriptionSv: () => faker.lorem.sentence(),
      alarmActive: false,
      removedAt: null,
      imageUrl: null,
    },
  ...
  position: {
    data: {
      ...
      nameSv: () => faker.person.jobTitle(),
      nameEn: () => faker.person.jobTitle(),
      descriptionSv: () => faker.lorem.paragraph(),
      email: (ctx) =>
        faker.internet
          .email({
            firstName: ctx.data.nameSv,
            lastName: "",
            provider: "dsek.se",
          })
          .toLowerCase(),
  ...
  markdown: {
    data: {
      markdownSv: () => faker.lorem.paragraphs({ min: 3, max: 6 }),
