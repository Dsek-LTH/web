...
<div
	class="bg-base-300 max-lg:border-t-primary lg:border-t-secondary flex flex-col gap-2 rounded-2xl border-t-4 p-4 max-md:flex-col md:flex-1"
>
		...
		{#each issues as issue (issue.id)}
			<li class="pop-out inline-block w-full">
				<a href={`/readme/${issue.year}/${issue.number}`}>
					<span
						class="i-mdi-newspaper max-lg:text-primary lg:text-secondary size-6"
					></span>
