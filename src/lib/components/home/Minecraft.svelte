...
<div
	class="bg-base-300 flex-col justify-between gap-2 rounded-t-2xl max-md:flex-col md:flex-1"
>
	<div class="flex w-full flex-row overflow-hidden rounded-t-2xl">
		{#snippet minecraft_border()}
			<svg class="fill-primary -mr-px w-full" viewBox="0 0 4.2333333 1.0583333">
				<path
					id="rect17"
...
<div class="bg-base-200 flex h-full flex-col justify-between rounded-b-xl p-4">
	{m.minecraft_server_description()}
	<div class="flex w-full justify-around">
		<a class="btn btn-ghost btn-sm" href="https://dynmap.dsek.se"
			><span class="i-mdi-map text-primary text-lg"
			></span>{m.minecraft_dynmap()}</a
		><a class="btn btn-ghost btn-sm" href="https://whitelist.dsek.se"
			><span class="i-mdi-playlist-add text-primary text-lg"
			></span>{m.minecraft_join_prompt()}</a
		>
	</div>
	<div class="bg-base-300 m-2 flex flex-row justify-center rounded-xl p-3">
