...
<div class="mx-auto md:container md:mt-8 md:grid md:grid-cols-2">
	...
	<main class="m-4 flex flex-col gap-4">
		...
		<div
			class="text-base-content/60 -mt-2 flex flex-wrap items-center gap-2 text-sm *:flex *:items-center *:gap-1"
		>
			<p>
		...
		{#if consumable.questionResponses.length > 0}
			<section
				class="rounded-box bg-base-200 space-y-2 self-start p-4 shadow-lg"
			>
