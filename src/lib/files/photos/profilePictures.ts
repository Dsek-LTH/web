...
const removeMyProfilePicture = (fileName: string, ctx?: AuthUser) => {
  const bucket = "members";
  if (!ctx?.studentId) error(401, "You are not logged in");
  if (!fileName.includes(ctx.studentId))
    error(403, "You are not allowed to delete this file");
