...
<div class="drawer-side z-20">
	...
	<ul class="menu bg-base-200 min-h-full min-w-60 p-0 font-semibold">
		<div class="flex gap-2 p-2 pb-0">
			<DarkLightToggle class="bg-base-300 flex-1 *:size-6" />
			<LanguageSwitcher class="bg-base-300 flex-1">
									...
									{#each route.children as child (child.title)}
										{#if !child.accessRequired || isAuthorized(child.accessRequired, $page.data.user)}
											<li class="py-1">
												...
												>
													<span class={`${child.icon} text-primary size-6`}
													></span>
													{child.title}
												</a>
											</li>
