<script lang="ts">
	import PostRevealDesktopLeftNavbar from "./PostRevealDesktopLeftNavbar.svelte";
	import { getRoutes } from "./routes";

	$: routes = getRoutes();

	const prefix = "/nollning";
	let checkbox: HTMLInputElement;
</script>

<div class="drawer">
	<input
		bind:this={checkbox}
		id="my-drawer-3"
		type="checkbox"
		class="drawer-toggle"
	/>
	<div class="drawer-content flex flex-col">
		<!-- Navbar -->
		<div
			class="navbar bg-base-100 sticky top-0 z-20 shadow-[0_4px_4px_#191B2740]"
		>
			<label
				for="my-drawer-3"
				aria-label="open sidebar"
				class="btn btn-circle bg-base-200 aspect-square size-10 !p-0 lg:hidden"
			>
				<span class="i-mdi-menu text-base-100 size-7"></span>
			</label>
			<div class="relative container mx-auto">
				<ul
					class="menu menu-horizontal hidden w-full justify-center gap-2 lg:flex"
				>
					{#each routes as route}
						<li>
							<a
								class="btn btn-ghost text-xl font-bold"
								href={`${prefix}${route.path}`}
							>
								{route.title}
							</a>
						</li>
					{/each}
				</ul>
				<PostRevealDesktopLeftNavbar />
			</div>
		</div>
		<!-- Page content here -->
		<main class="*:scrollbar-hide bg-base-100 relative flex-1">
			<!-- so absolute positioning is outside padding -->
			<div class="scrollbar-hide px-6 py-6">
				<slot />
			</div>
		</main>
	</div>
	<div class="drawer-side z-30">
		<label for="my-drawer-3" aria-label="close sidebar" class="drawer-overlay"
		></label>
		<ul class="menu bg-base-100 min-h-full w-80 p-4">
			<!-- Sidebar content here -->
			{#each routes as route}
				<li>
					<a
						on:click={() => (checkbox.checked = false)}
						class="btn btn-ghost justify-start text-xl font-bold"
						href={`${prefix}${route.path}`}
					>
						{route.title}
					</a>
				</li>
			{/each}
		</ul>
	</div>
</div>
