...
export const load = async ({ parent, url }) => {
  ...
  const type: DocumentType = (url.searchParams.get("type") as DocumentType) || "guild-meeting";
  const files = await fileHandler.getInBucket(session?.user, "documents", year + "/" + (prefixByType[type] ?? ""), true);
