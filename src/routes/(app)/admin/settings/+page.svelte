...
<div class="flex flex-col gap-4">
	{#if (editingSetting === null && isAuthorized(apiNames.ADMIN.SETTINGS.CREATE, data.user)) || isAuthorized(apiNames.ADMIN.SETTINGS.UPDATE, data.user)}
		<form
			class="form-control rounded-box bg-base-300 gap-2 p-4"
			method="POST"
			use:updateEnhance
			action="?/update"
		>
	...
	<table class="rounded-box bg-base-300 table">
		<thead>
			<tr>
				<th>Key</th>
				<th>Value</th>
	...
	{#if data.nollning && isAuthorized(apiNames.ADMIN.SETTINGS.UPDATE, data.user)}
		<form
			...
			use:nollningEnhance
			class="form-control rounded-box bg-base-300 justify-between gap-2 p-4 md:flex-row"
		>
			<Labeled
