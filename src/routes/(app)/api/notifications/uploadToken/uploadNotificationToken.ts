...
const cache = createCache(); // contains a map from tokens to the corresponding member id which has been set
...
const CACHE_TTL = 7 * 24 * 60 * 60 * 1000; // one week
...
export const uploadNotificationToken = async (
  ...
  if (await cache.get(token) == user.memberId) return;
  try {
    ...
    if (existing) {
      cache.set(token, user.memberId, CACHE_TTL);
      return;
    }
