...
<tr>
	...
	<th>
		{#if consumable?.consumedAt === null}
			<RowAction action="?/consume" consumableId={consumable.id}>
				<span class="i-mdi-flame-circle text-secondary text-xl"></span>
			</RowAction>
		{:else if consumable}
			<RowAction
				action="?/unconsume"
			...
			>
				<span class="i-mdi-redo-variant text-error text-xl"></span>
			</RowAction>
		{:else}
	...
	<th>
		{#if consumable}
			...
			>
				<span class="i-mdi-cash-refund text-success text-xl"></span>
			</RowAction>
		{/if}
	</th>
</tr>
