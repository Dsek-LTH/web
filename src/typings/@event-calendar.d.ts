// TODO: Remove this file once the following PR is merged: https://github.com/DefinitelyTyped/DefinitelyTyped/pull/70317
...
declare module "@event-calendar/core" {
	class Calendar {
		...
		setOption<K extends keyof Calendar.Options>(
			option: K,
			value: Calendar.Options[K],
		): Calendar;
		addEvent(
			event: Calendar.Event | Calendar.EventInput,
		): Calendar.Event | null;
	...
	namespace Calendar {
		...
		type DomEvent =
			GlobalEventHandlersEventMap[keyof GlobalEventHandlersEventMap];
		...
		interface Options {
			...
			eventClassNames?:
				| string
				| string[]
				| ((info: EventClassNamesInfo) => string | string[]);
			eventClick?: (info: EventClickInfo) => void;
			eventColor?: string;
			eventContent?:
				| Content
				| ((info: EventContentInfo) => Content | undefined);
